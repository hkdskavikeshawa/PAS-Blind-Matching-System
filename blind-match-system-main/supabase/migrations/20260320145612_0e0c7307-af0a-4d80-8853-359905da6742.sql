...
CREATE TYPE public.app_role AS ENUM ('student', 'supervisor', 'module_leader', 'system_admin');
...
CREATE POLICY "Admins and module leaders can view all profiles"
  ON public.profiles FOR SELECT
  USING (
    public.has_role(auth.uid(), 'system_admin') OR
    public.has_role(auth.uid(), 'module_leader')
  );
...
CREATE POLICY "Module leaders can view all roles"
  ON public.user_roles FOR SELECT
  USING (public.has_role(auth.uid(), 'module_leader'));
